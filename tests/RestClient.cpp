...
RestClient::RestClient() : sslOptions(std::make_unique<cpr::SslOptions>()) {
  auto caInfo = [&]() -> std::string {
    ...
    std::cout << certPath.string() <<
        ...
        "https://curl.se/ca/cacert.pem." << std::endl;*/
    return "";
  }();
...
#ifdef DEBUG
  std::cout << "Posting to " << remoteUrl << "/" << path
            << " with data = " << post.dump() << std::endl;
#endif
...
void RestClient::put(const std::string_view remoteUrl,
  ...
  // Allow caller to disable logging for things like passwords/tokens
#ifdef DEBUG
  std::cout << "Putting to" << remoteUrl << "/" << path
            << " with data = " << putData.dump() << std::endl;
#endif
  auto actualPath = std::string(remoteUrl) + std::string(path);
...
void RestClient::del(const std::string_view remoteUrl,
  ...
  auto actualPath = std::string(remoteUrl) + std::string(path);
#ifdef DEBUG
  std::cout << "Delete resource at path" << remoteUrl << "/" << path
            << std::endl;
#endif
...
#ifdef DEBUG
  std::cout << "Downloading" << r.text.size() << "bytes from" << remoteUrl
