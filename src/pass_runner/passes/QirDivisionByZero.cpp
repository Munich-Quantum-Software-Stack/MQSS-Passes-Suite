 ...
 * @brief TODO
 */
const char* const QirDivisionByZeroPass::EC_REPORT_FUNCTION      = "__qir__report_error_value";
...
PreservedAnalyses QirDivisionByZeroPass::run(Module &module, ModuleAnalysisManager &MAM) {
    ...
    for (auto instr : instructions) {
        ...
        auto cmp            = builder.CreateICmp(CmpInst::Predicate::ICMP_EQ, op2, builder.getInt64(0));
    ...
    if (entry) {
        for (auto start_block : exit_blocks) {
            ...
            auto cmp            = builder.CreateICmp(CmpInst::Predicate::ICMP_NE, load, builder.getInt64(0));
            auto old_terminator = start_block->getTerminator();
...
void QirDivisionByZeroPass::raiseError(int64_t error_code, Module &module, Instruction* instr)
{
    ...
    builder.SetInsertPoint(start_block->getTerminator());
    LoadInst* load = builder.CreateLoad(builder.getInt64Ty(), error_variable_);
    auto  cmp  = builder.CreateICmp(CmpInst::Predicate::ICMP_EQ, load, builder.getInt64(0));
...
/**
 * @brief External function for loading the 'QirDivisionByZeroPass' as a 'PassModule'.
 * @return QirDivisionByZeroPass
 */
extern "C" PassModule* loadQirPass() {
	    return new QirDivisionByZeroPass();
}
