...
void loadRotationGatesToQC(Operation *op, qc::QuantumComputation &qc){
  if (isa<quake::RxOp>(op) || isa<quake::RyOp>(op) || isa<quake::RzOp>(op)){
    ...
    qubit= supportQuake::extractIndexFromQuakeExtractRefOp(operand2.getDefiningOp());
    #ifdef DEBUG
      llvm::errs() << "Operation ";
      op->print(llvm::errs());
      llvm::errs() <<"\n";
...
void loadXYZGatesToQC(Operation *op, qc::QuantumComputation &qc){
  if (isa<quake::XOp>(op) || isa<quake::YOp>(op) || isa<quake::ZOp>(op)){
    ...
    if (op->getOperands().size() ==2){
      ...
      Value operand1 = op->getOperands()[0];
      qubit_ctrl = supportQuake::extractIndexFromQuakeExtractRefOp(operand1.getDefiningOp());
      Value operand2 = op->getOperands()[1];
      qubit_target = supportQuake::extractIndexFromQuakeExtractRefOp(operand2.getDefiningOp());
      #ifdef DEBUG
        llvm::errs() << "Operation ";
        op->print(llvm::errs());
        llvm::errs() <<"\n";
    ...
    if (op->getOperands().size() ==1){
      Value operand1 = op->getOperands()[0];
      int qubit= supportQuake::extractIndexFromQuakeExtractRefOp(operand1.getDefiningOp());
      #ifdef DEBUG
        llvm::errs() << "Operation ";
        op->print(llvm::errs());
        llvm::errs() <<"\n";
