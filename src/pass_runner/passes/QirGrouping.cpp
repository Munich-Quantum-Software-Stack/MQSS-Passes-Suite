 ...
 * @param block TODO
 */
void QirGroupingPass::prepareSourceSeparation(Module &module,
                                              BasicBlock *block) {
  ...
  pre_classical_block_ = BasicBlock::Create(context, "pre-quantum",
                                            block->getParent(), quantum_block_);
  ...
  block->replaceUsesWithIf(pre_classical_block_, [](Use &use) {
    ...
    if ((instr_class & SourceQuantum) != 0) {
      ...
      if (instr_class == TransferQuantumToClassical) {
        ...
        for (auto &op : instruction.operands()) {
          ...
          // Consider for instance, %a1.i.i = select i1 %0, %Qubit* null,
          // %Qubit* inttoptr (i64 1 to %Qubit*) which could refer to qubit 0 or
          // qubit 1 depending on %0. If %0 is not known at compile time, we
      ...
      if (call_instr != nullptr) {
        ...
        is_post_quantum_instruction |=
            (name.size() >= qir_runtime_prefix.size() &&
             name.substr(0, qir_runtime_prefix.size()) == qir_runtime_prefix);
      }
      ...
      assert(
          ((void)"Unsupported occurring while grouping instructions", false));
  }
...
void QirGroupingPass::expandBasedOnDest(Module &module, BasicBlock *block,
  ...
  block->replaceUsesWithIf(extra_block, [](Use &use) {
    auto *phi_node = dyn_cast<PHINode>(use.getUser());
    return (phi_node == nullptr);
  });
...
QirGroupingPass::Result QirGroupingPass::runGroupingAnalysis(Module &module) {
  ...
  for (auto &function : module) {
    for (auto &block : function) {
      ...
      // Differentiating between blocks that has measurements and those that has
      // not
      if (!has_measurement)
