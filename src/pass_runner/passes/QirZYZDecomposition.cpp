                                                ...
                                                ComplexMatrix theGate)
{
    ...
    Value *phiValue =
        ConstantFP::get(context, APFloat(static_cast<float>(phi)));
    Value *lamValue =
        ConstantFP::get(context, APFloat(static_cast<float>(lam)));
    Value *thetaValue =
        ConstantFP::get(context, APFloat(static_cast<float>(theta)));

    return {phiValue, thetaValue, lamValue};
}

std::vector<double>
QirZYZDecompositionPass::getDecompositionAnglesAsNumber(LLVMContext &context,
                                                        ComplexMatrix theGate)
{
            ...
            for (auto &instruction : block)
            {
                ...
                for (std::string gateToDecompose : gatesToDecompose)
                {
                    if (gateToDecompose == calledFunction->getName())
                    {
                        ...
                        std::vector<Value *> theAngles = getDecompositionAngles(
                            rContext, theGate); // phi lamda theta
...
/**
 * @brief External function for loading the 'QirZYZDecompositionPass' as a
 * 'PassModule'.
