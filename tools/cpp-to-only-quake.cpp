...
// Function to check if a `func.func` operation has the `"cudaq-kernel"` attribute
bool isCudaqKernel(mlir::func::FuncOp funcOp) {
  auto attrs = funcOp->getAttrDictionary();
  return attrs.get("cudaq-kernel") != nullptr;
}

// Function to collect the `func.func` operations that are CUDA-Q kernels as a string
...
int main(int argc, char *argv[])
{
  ...
  desc.add_options()
      ...
      ("input", po::value<std::string>(), "The input file might be: *.cpp (cpp only works if cudaq compiler is installed!)")
      ("output", po::value<std::string>(), "The name of the output file. It should be *.qke");
