...
void ReplaceHZHToX(mlir::Operation *currentOp) {
  ...
  rewriter.create<quake::XOp>(currentGate.getLoc(), 
  ...
  class HZHToXPass
      : public PassWrapper<HZHToXPass , OperationPass<func::FuncOp>> {
  public:
    MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(HZHToXPass)
  
    llvm::StringRef getArgument() const override { return "h-z-h-to-x"; }
    llvm::StringRef getDescription() const override { return "Optimization pass that replaces a pattern composed of H, Z, H by X";}
