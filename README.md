...
2. Install the FoMaCs, the QDMI, the QRM, and the Backends libraries:
   - Clone the project:
      ```bash
      git clone git@github.com:Munich-Quantum-Software-Stack/QRM.git qrm.git
      ```
   - Navigate to the `qrm.git` directory (if you are not already there) and move to the right branch:
      ```bash
      cd qrm.git
...
3. Navigate to the `passes.git` directory (if you are not already there):
   ```bash
   cd passes.git/
   ```
...
4. Run `make` to install the passes as shared libraries and add the installation path to the `PASSES` environment variable:
   ...
   - One may also specify the directory where the build files can be written to. Note that the equivalent command to the one above is:
      ```bash
      export INSTALLATION_PATH=installation/path/passes
...
1. Navigate to the `passes.git` directory (if you are not already there):
   ```bash
   cd passes.git/
   ```
