...
int main(void) {
  ...
  ssize_t bytesSent =
      send(clientSocket, &fileSizeNetwork, sizeof(fileSizeNetwork), 0);
  if (bytesSent == -1) {
    std::cerr
        << "[Selector] Error: Failed to send size of generic QIR to the QPR"
        << std::endl;
    exit(1);
  }
  bytesSent = send(clientSocket, genericQir, fileSize, 0);
  if (bytesSent == -1) {
    std::cerr << "[Selector] Error: Failed to send generic QIR to the QPR"
              << std::endl;
    exit(1);
  }
  delete[] genericQir;
  ...
  std::vector<std::string> passes{
      ...
      "libQirGroupingPass.so", // TODO: Does __quantum__rt__initialize belong to
                               // post-quantum?
  ...
  while (!passes.empty()) {
    ...
    if (send(clientSocket, &passSizeNetwork, sizeof(passSizeNetwork), 0) < 0) {
      std::cout << "[Selector] Warning: Failed to send size of the following "
                   "pass to the QPR: \n"
                << pass << std::endl;
      continue;
    }
    if (send(clientSocket, libPass, strlen(libPass), 0) < 0) {
      std::cout << "[Selector] Warning: Failed to send the followig pass to "
                   "the QPR: \n"
                << pass << std::endl;
      continue;
    }
  ...
  if (send(clientSocket, &eotSizeNetwork, sizeof(eotSizeNetwork), 0) < 0) {
    std::cout << "[Selector] Warning: Failed to send size of the EOT to the QPR"
              << std::endl;
    close(clientSocket);
    return 1;
  }
  if (send(clientSocket, eot, strlen(eot), 0) < 0) {
    std::cerr
        << "[Selector] Error: failed to send end of transmission to the QPR"
        << std::endl;
    close(clientSocket);
    return 1;
  }
