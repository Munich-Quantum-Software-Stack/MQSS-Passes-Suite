...
void ReplaceHXHToZ(mlir::Operation *currentOp) {
  ...
  rewriter.create<quake::ZOp>(currentGate.getLoc(), 
  ...
  class HXHToZPass
      : public PassWrapper<HXHToZPass , OperationPass<func::FuncOp>> {
  public:
    MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(HXHToZPass)
  
    llvm::StringRef getArgument() const override { return "h-x-h-to-z"; }
    llvm::StringRef getDescription() const override { return "Optimization pass that replaces a pattern composed of H, X, H by Z";}
