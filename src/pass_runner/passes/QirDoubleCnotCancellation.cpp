 ...
 * @brief Implementation of the 'QirDoubleCnotCancellationPass' class. <a href="https://gitlab-int.srv.lrz.de/lrz-qct-qis/quantum_intermediate_representation/qir_passes/-/blob/Plugins/src/passes/QirDoubleCnotCancellation.cpp?ref_type=heads">Go to the source code of this file.</a>
 *
 ...
 */
PreservedAnalyses QirDoubleCnotCancellationPass::run(Module &module, ModuleAnalysisManager &/*MAM*/) {
                        ...
                        if (prev_instruction) {
                            ...
                            if (prev_function) {
                                ...
                                if (previous_name == "__quantum__qis__cnot__body") {
                                    if (prev_instruction->getArgOperand(0) == current_instruction->getArgOperand(0) 
                                        && prev_instruction->getArgOperand(1) == current_instruction->getArgOperand(1)) {
                                        gatesToRemove.push_back(prev_instruction);
                                        gatesToRemove.push_back(current_instruction);
                            
                                        errs() << "              A pair of Cnot gates found.\n";
                                    }
...
/**
 * @brief External function for loading the 'QirDoubleCnotCancellationPass' as a 'PassModule'.
