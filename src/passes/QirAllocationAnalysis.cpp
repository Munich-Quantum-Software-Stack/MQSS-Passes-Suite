            ...
            for (uint64_t i = 0; i < instr.getNumOperands(); ++i) {
                ...
                switch (type)
                {
					case ResourceType::QubitResource:
						...
						result.largest_qubit_index = result.largest_qubit_index < n ? n : result.largest_qubit_index;

						break;
					case ResourceType::ResultResource:
						results_used.insert(n);
						result.largest_result_index = result.largest_result_index < n ? n : result.largest_result_index;
						break;
