  ...
  for (auto &block : function) {
    for (auto &instr : block) {
      for (uint64_t i = 0; i < instr.getNumOperands(); ++i) {
        ...
        // Checking if it is a qubit resource reference and extracting the
        // corresponding id and type. Otherwise, we skip to the next operand.
        ...
        case ResourceType::QubitResource:
          ...
          result.largest_qubit_index =
              result.largest_qubit_index < n ? n : result.largest_qubit_index;

          break;
        case ResourceType::ResultResource:
          results_used.insert(n);
          result.largest_result_index =
              result.largest_result_index < n ? n : result.largest_result_index;
          break;
